...
// MakePointZero
func MakePointZero() *Point {
	return &Point{0,0,0}
}

// MakePointX returns the unit X axis.
func MakePointX() *Point {
	return &Point{1, 0, 0}
}

// MakePointY returns the unit Y axis.
func MakePointY() *Point {
	return &Point{0, 1, 0}
}

// MakePointZ returns the unit Z axis.
func MakePointZ() *Point {
	return &Point{0, 0, 1}
}

// MakePointNormal returns the normal for a slice of points.
// Can be used for e.g. backface culling or shading.
func MakePointNormal(points []Point) *Point {
	pointsLen := len(points)
	if pointsLen < 2 {
		return MakePointZ() // Default normal
	}
	p0 := &points[0]
	p1 := &points[1]
...
func (l *Point) SubtractAssign(r *Point) {
	l.X -= r.X
	l.Y -= r.Y
	l.Z -= r.Z
}
...
// AddAssign will add another Point's X,Y,Z components and assign it to the Point
func (l *Point) AddAssign(r *Point) {
	l.X += r.X
	l.Y += r.Y
	l.Z += r.Z
}
...
// ScaleAssign multiplies the X,Y,Z components with a scalar value and assigns the result to the receiver.
func (p *Point) ScaleAssign(s float64) {
	p.X *= s
	p.Y *= s
	p.Z *= s
}
...
func (l *Point) CrossAssign(r *Point) {
	l.X, l.Y, l.Z = l.Y*r.Z-l.Z*r.Y, l.Z*r.X-l.X*r.Z, l.X*r.Y-l.Y*r.X
}
