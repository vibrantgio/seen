...
// ---------------------------------------------------------------------------
// Projection math, mirrored from layer/bsort/layer.go RenderOn.
// ---------------------------------------------------------------------------

// projectPoint maps a world point to screen coordinates exactly as the renderer
// does: projection -> clip (reject when clip-space z <= -2) -> perspective
// divide -> postscale.
...
// eyePoint recovers the eye location the renderer uses for back-to-front order.
...
// renderScene builds a scene from the face specs and renders it through the SVG
// bsort pipeline, returning the document. Every face gets a flat fill so it is
// always emitted; ShowBackfaces is on so backface culling never drops a face
// (isolating the ordering bug). The Flat shader ignores lights, so the emitted
// fill color equals the face's material color exactly.
func renderScene(t *testing.T, id string, faces []faceSpec) *svg.SVG {
	t.Helper()
	...
	ctx := svg.NewContext(doc.GetElementById(id), bsort.NewLayerForScene(scene))
	if ctx == nil {
...
// renders correctly today. It guards the harness against false positives.
...
func TestArtifact(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.scene, func(t *testing.T) {
			doc := renderScene(t, "artifact-"+tc.scene, tc.faces)
			...
			for _, s := range tc.samples {
				...
				if got != s.expect {
					t.Errorf("%s/%s: at screen (%.3f,%.3f) painter's order is wrong: "+
						"expected face %q nearest the eye, but face %q is painted last (covers the pixel). "+
						"covering=%d. The bsort BSP did not split straddling polygons.",
						tc.scene, s.label, sx, sy, s.expect, got, covering)
				}
			}
		})
	}
