...
// TestNewTreeNoSplit: whole-polygon mode never cuts. Every input polygon —
// including ones the splitting builder would cut, like the interpenetrating
// cross — comes back exactly once, unmarked and with its original ring, and
// the caller's slice is not mutated. The cross also pins the contrast: the
// splitting NewTree does produce pieces for it, so a regression that quietly
// turns splitting off would show up here first.
func TestNewTreeNoSplit(t *testing.T) {
	cross := []bsp.Plane{
		planeWith(1, point.Pt(-120, -120, -120), point.Pt(120, -120, 120),
			point.Pt(120, 120, 120), point.Pt(-120, 120, -120)),
		planeWith(2, point.Pt(-120, -120, 120), point.Pt(120, -120, -120),
	...
	countPieces := func(tree *bsp.Tree) (whole map[int]int, pieces int) {
		whole = map[int]int{}
		var walk func(tr *bsp.Tree)
		walk = func(tr *bsp.Tree) {
			if tr == nil {
	...
	if _, pieces := countPieces(bsp.NewTree(cross)); pieces == 0 {
		t.Fatal("contrast broken: the splitting NewTree no longer cuts the cross scene")
	}
	...
	for name, planes := range scenes {
		t.Run(name, func(t *testing.T) {
			whole, pieces := countPieces(bsp.NewTreeNoSplit(planes))
			if pieces != 0 {
				t.Errorf("NewTreeNoSplit produced %d pieces, want 0", pieces)
			}
			for _, p := range planes {
				if whole[p.Id] != 1 {
					t.Errorf("face %d appears %d times, want exactly 1 (whole)", p.Id, whole[p.Id])
				}
			}
			if len(whole) != len(planes) {
				t.Errorf("tree holds %d whole faces, want %d", len(whole), len(planes))
			}
			for i := range planes {
				if planes[i].NoSplit {
					t.Errorf("input slice mutated: planes[%d].NoSplit set", i)
				}
			}
		})
	}
