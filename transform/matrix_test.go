...
func TestMatrixFromDualQuat(t *testing.T) {
	...
	dq := MakeDualQuatRXYZ(MakeQuatAxisAngle(1, 2, 3, math.Pi/2.0), 4, 5, 6)
	m := dq.Mat3x4()
...
func TestMat3x4Multiplication(t *testing.T) {
	...
	dq1 := MakeDualQuatRXYZ(MakeQuatAxisAngle(1,2,3, math.Pi/2.0), 4, 5, 6)
	dq2 := MakeDualQuatRXYZ(MakeQuatAxisAngle(0,1,0, math.Pi/4.0), 10, 11, 12)
	dq := dq1.Mul(dq2)
...
func TestMat4x4Multiplication(t *testing.T) {
	...
	dq1 := MakeDualQuatRXYZ(MakeQuatAxisAngle(1,2,3, math.Pi/2.0), 4, 5, 6)
	dq2 := MakeDualQuatRXYZ(MakeQuatAxisAngle(0,1,0, math.Pi/4.0), 10, 11, 12)
	dq3 := MakeDualQuatRXYZ(MakeQuatAxisAngle(0,1,1, math.Pi/3.0), 100, 110, 120)
