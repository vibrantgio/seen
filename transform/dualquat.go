...
func MakeDualQuatXYZ(x, y, z float64) *DualQuaternion {
	return &DualQuaternion{IdentQuaternion, &Quaternion{0.5 * x, 0.5 * y, 0.5 * z, 0}}
}

func MakeDualQuatR(r *Quaternion) *DualQuaternion {
	return &DualQuaternion{r, ZeroQuaternion}
}

func MakeDualQuatRXYZ(r *Quaternion, x, y, z float64) *DualQuaternion {
...
// By comparison a homogenous matrix transform takes 9 muls and 9 adds but needs to have the Mat3x4 extracted first.
...
// Mat3x4 will return a matrix with 3 rows and 4 columns, the left 3x3 matrix
// contains the rotation and the right 3x1 vector contains the translation.
// It takes 38 muls, 28 adds to derive a homogenous matrix from the dual quaternion.
func (q *DualQuaternion) Mat3x4() *Mat3x4 {
