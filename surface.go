...
type Surface struct {
	...
	*Shape
	...
	// Dirty flag can be set whenever the render.Surface generated from
	// the seen.Surface needs to be regenerated.
	Dirty bool
...
func NewSurfaceWith(points Points) *Surface {
	s := &Surface{}
	s.Id = UniqueId()
...
func (s *Surface) SetFill(value interface{}) (err error) {
	s.FillMaterial, err = NewMaterialWith(value)
	return
}

func (s *Surface) SetStroke(value interface{}) (err error) {
	s.StrokeMaterial, err = NewMaterialWith(value)
	return
}
